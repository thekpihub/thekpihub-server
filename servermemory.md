...
## 2026-09-13 — Rocket.new-linked Vercel project deleted (credential-exposure cleanup, phase 1)

User spotted a "No Deployment" warning on `thekpihub-server.vercel.app` in their own Vercel
dashboard (screenshot) and asked for it to be fixed. Before touching anything, confirmed via
`get_project` that this was the *same* flagged project from
[[rocketnew-thekpihub-server-credential-exposure]] — `prj_nYjRHCK4wUpgUpstx9KjH16NLsmg`, `live:
false`, one failed production deployment, linked to `hsharmagxi-debug/thekpihub-server` (the
personal-account Rocket.new KPI-dashboard rebuild, archived 2026-09-09) — not any part of the
real KPI Hub pipeline. That project's env vars still held all 27 `Credentials/.env` secret
names.

Flagged this explicitly rather than just deploying/dismissing it, since "fix it" had two very
different real meanings (delete vs. actually deploy a credential-loaded abandoned app live).
User's choice, given three options: **delete the Vercel project**.

Executed: `DELETE https://api.vercel.com/v9/projects/prj_nYjRHCK4wUpgUpstx9KjH16NLsmg?teamId=team_tu9mNsxlpoUVyUnkIMGWhk1C`
using `VERCEL_ACCESS_TOKEN` from `Credentials/.env` → `204`. Verified via a follow-up
`get_project` call on the same ID → `404 Not Found`. The Vercel-side exposure (a live project
surface holding those 27 secret names as env vars) is now fully gone.

**User's explicit instruction this same session: "credentials are good for now"** — i.e. do NOT
proceed with the rotation checklist in [[rocketnew-thekpihub-server-credential-exposure]] right
now. Still genuinely open for a future session, only when asked:
- The archived GitHub repo `hsharmagxi-debug/thekpihub-server` still exists (archived, not
  deleted).
- Credential rotation status unchanged since 2026-09-05: only `SUPABASE_DB_PASSWORD` confirmed
  rotated. `GITHUB_ACCESS_TOKEN`/`HOSTINGER_ACCESS_TOKEN`/`RAILWAY_ACCESS_TOKEN` still can't be
  rotated via API (need new tokens generated by the user in each dashboard first).
  `WP_DB_PASSWORD`/Resend key/`WINGCOMMANDER_BYOK_ENCRYPTION_KEY` still unconfirmed either way.
