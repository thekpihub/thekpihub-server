...
function PulseMock() {
  ...
  const dotX = dotK * W;
  const dotY = H / 2 + Math.sin(dotX * 0.04 + t * 0.003) * 22 + Math.sin(dotX * 0.015 + t * 0.0014) * 12;
...
function AnomalyMock() {
  ...
  const dropX = (32 / seg) * W;
  const dropY = H - 40 - Math.sin(32 * 0.25) * 8 - 32 * 1.6 + 52;
...
function FeatureBlock({ num, name, tm, claim, body, flip, Mock, annot }) {
  return (
    <div className={`lp-featblk ${flip ? 'lp-featblk--flip' : ''} lp-section-host`}>
      <div className="lp-annot">{annot}</div>
      <div>
        <div className="lp-featblk-num">{num}</div>
        <h3 className="lp-featblk-name">{name}<sup className="lp-featblk-tm">{tm}</sup></h3>
        <div className="lp-featblk-claim">{claim}</div>
        <p className="lp-featblk-body">{body}</p>
      </div>
      <Mock />
    </div>
  );
...
function SectionFeatures() {
  return (
    <section className="section lp-section-host" data-section="features" data-screen-label="§5 Features">
      <div className="lp-annot">§5 · Features · Alternating L/R · Micro-UI per feature</div>
      <div className="section-inner">
        <div className="section-head">
          <div>
            <span className="eyebrow">What others haven't built</span>
            <h2>We didn't improve what existed.<br/>We built <em>what didn't.</em></h2>
          </div>
...
function SectionProof() {
  return (
    <section className="section alt lp-section-host" data-section="proof" data-screen-label="§6 Proof">
      <div className="lp-annot">§6 · Social proof · Editorial pull-quote + metric bar</div>
      <div className="section-inner">
        <div className="lp-quote">
          <span className="eyebrow">Trusted by people who run on performance</span>
          <div style={{ height: 28 }} />
          <div className="lp-quote-mark">"</div>
          <div className="lp-quote-text">
            The first platform where I actually <em style={{ color: 'var(--gold)' }}>looked forward</em> to checking my numbers.
          </div>
          <span className="lp-quote-attr">Placeholder · Founding customer voice · populate at launch</span>
        </div>

        <div className="lp-proof-bar">
          <div className="lp-proof-item"><span className="lp-proof-star">✦</span><span className="lp-proof-lbl">4 Role-Intelligent Dashboards</span></div>
          <div className="lp-proof-item"><span className="lp-proof-star">✦</span><span className="lp-proof-lbl">100% Automated Reporting</span></div>
          <div className="lp-proof-item"><span className="lp-proof-star">✦</span><span className="lp-proof-lbl">Live Production Platform</span></div>
          <div className="lp-proof-item"><span className="lp-proof-star">✦</span><span className="lp-proof-lbl">Built for Scale from Day One</span></div>
        </div>
      </div>
    </section>
  );
